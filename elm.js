...
Elm.Import.make = function (_elm) {
   ...
   if (_elm.Import.values)
   return _elm.Import.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var search = function (file) {
      return function () {
         ...
         return A3($Regex.find,
         $Regex.All,
         pattern,
         file);
      }();
   };
   var exposes = function (s) {
      return _U.eq(s,
      "") ? $Maybe.Nothing : $Maybe.Just($List.map($String.trim)(A2($String.split,
      ",",
      s)));
   };
   ...
   var process = function (submatches) {
      return function () {
         ...
         var name = A2($Maybe.withDefault,
         "",
         join(A2($Array.get,
         0,
         submatches$)));
         var alias = join(A2($Array.get,
         1,
         submatches$));
         var exposedStart = A2($Maybe.withDefault,
         "",
         join(A2($Array.get,
         2,
         submatches$)));
         var exposedEnd = A2($Maybe.withDefault,
         "",
         join(A2($Array.get,
         3,
         submatches$)));
         var exposed = exposes($String.trim(A2($Basics._op["++"],
         exposedStart,
         exposedEnd)));
         return {_: {}
                ,alias: alias
   ...
   var None = {ctor: "None"};
   var Import = F2(function (a,b) {
      return {_: {}
             ,alias: a
             ,exposed: b};
   });
   var toImport = function (_v2) {
      return function () {
         return function () {
            var exposedSet = function () {
               ...
               {case "Just":
                  switch (_v4._0.ctor)
                    {case "::": switch (_v4._0._0)
                         {case "..":
                            switch (_v4._0._1.ctor)
                              {case "[]": return Every;}
                              break;}
                         break;}
                    return Some($Set.fromList(_v4._0));
   ...
   var defaultImports = $Dict.fromList(_L.fromArray([A2(_op["=>"],
                                                    "Basics",
                                                    Every)
                                                    ,A2(_op["=>"],
                                                    "List",
                                                    Some($Set.fromList(_L.fromArray(["List"
                                                                                    ,"::"]))))
                                                    ,A2(_op["=>"],
                                                    "Maybe",
                                                    Some($Set.singleton("Maybe")))
                                                    ,A2(_op["=>"],
                                                    "Result",
                                                    Some($Set.singleton("Result")))
                                                    ,A2(_op["=>"],
                                                    "Signal",
                                                    Some($Set.singleton("Signal")))]));
   var imports = function (rawImports) {
      return function () {
         var toDict = function (list) {
            return A2($Dict.union,
   ...
   var parse = function (source) {
      return imports($List.map(process)($List.map(function (_) {
         return _.submatches;
      })(search(source))));
   };
   _elm.Import.values = {_op: _op
                        ,parse: parse
                        ,Import: Import
                        ,None: None
                        ,Some: Some
...
Elm.Main.make = function (_elm) {
   ...
   var downloadDocs = function () {
      ...
      var pull = function (path) {
         return $Task.mapError(function (_v4) {
            return function () {
               return A2($Basics._op["++"],
		...
		Task.asyncFunction(function(callback) {
			http.get(url, function(res) {
				...
				res.on("end", function () {
					...
					return callback({ ctor: 'NetworkError', _0: err.message });
			});
		});
	}
...
Elm.Package.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var Vs = F3(function (a,b,c) {
      return {_: {}
             ,comment: b
             ,name: a
             ,signature: c};
   });
   var Values = F3(function (a,
   b,
   c) {
      return {_: {}
             ,aliases: a
   ...
   var $package = function (packageName) {
      return function () {
         ...
         A2($Json$Decode._op[":="],
         "types",
         $Json$Decode.list(type$)),
         A2($Json$Decode._op[":="],
         "values",
         $Json$Decode.list(v)));
         return $Json$Decode.list(A3($Json$Decode.object2,
         Module(packageName),
         name,
         values));
      }();
   };
   var decode = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return function () {
