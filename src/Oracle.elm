...
collate imports moduleList filterName =
  let
    ...
    accept ( token, info ) =
      if String.startsWith filterName token then
        Just info
      else
        Nothing
  in
  ...
  let
    ...
    nameToPair vname =
      let
        ...
        info =
          Info name fullName (urlTo name) vname.signature vname.comment

        localName =
          Maybe.withDefault modul.name alias ++ "." ++ name
